...
class TestTagRelease:
    ...
    def test_ingest_config(self):
        ...
        expected_config = [tag_releases.RepoInfo("test1", "develop", "develop1", "calver"),
                           tag_releases.RepoInfo("test2", "release", "release1", "semver", "1.2.4rc2")]

        config = tag_releases.ingest_config(pathlib.Path("test.toml"))
    ...
    def test_generate_repo_url(self):
        ...
        """
        config = tag_releases.ingest_config(pathlib.Path("test.toml"))
        assert(tag_releases.generate_repo_url(config[0]) == "https://api.github.com/repos/ssl-hep/test1")
        assert(tag_releases.generate_repo_url(config[1]) == "https://api.github.com/repos/ssl-hep/test2")
    ...
    def test_generate_tag(self):
        ...
        assert (tag_releases.generate_tag(config[1]) == f"1.2.4rc2-release1")
    ...
    def test_verify_branch(self):
        ...
        config = tag_releases.ingest_config(pathlib.Path("invalid.toml"))
